...
public abstract class PermissionHelper extends Fragment {
    ...
    public void izinIste(final String[] istenilenIzinler, final int requestCode) {
        ...
        for (String izin : istenilenIzinler) {
            izinKontrol = izinKontrol + ContextCompat.checkSelfPermission(getContext(), izin);
            mazeretGoster = mazeretGoster || ActivityCompat.shouldShowRequestPermissionRationale(getActivity(), izin);
        }
        ...
        if (izinKontrol != PackageManager.PERMISSION_GRANTED) {
            ...
            if (mazeretGoster) {
                ...
                builder.setPositiveButton("IZIN VERMEK ISTIYORUM", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        ActivityCompat.requestPermissions(getActivity(), istenilenIzinler, requestCode);
                    }
                });
                builder.show();
            } else {
